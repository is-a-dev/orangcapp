...
import random
_blobs = ['<:blobcat_cute:1236176247759966298>','<:blobcat_food:1236176414768762880>','<:blobcat_pat:1236176350738387027>','<:blobcat_present:1236175869404254209>','<:blobcat_queen:1236175473168486481>','<:blobcat_salute:1236175609391087646>','<:blobcat_smart:1236175952891871322>','<:blobcat_sup:1236175739859112048>','<:blobcat_thirst:1236176039898648606>']
class DigSelectOption(nextcord.SelectOption):
    def __init__(self, record_name: str):
        super().__init__(label=record_name, value=record_name, description=f"Fetch the {record_name} record of the domain", emoji=random.choice(_blobs))
...
class DigDropdown(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        try:
            ...
            answer = "NOT FOUND"
        await self._message.edit(embed=construct_embed(self._url, answer, interaction.data['values'][0]))
